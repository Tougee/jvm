...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/tougee/jvm/cmd/gomobile
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command("go", "list", "-f", "{{.Stale}}", "github.com/tougee/jvm/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/tougee/jvm/app").CombinedOutput()
	if err != nil {
